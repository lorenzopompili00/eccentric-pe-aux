...
class SEOBNRv6EHM(CompactBinaryCoalescenceGenerator):

    """
    Implements a wrapper for SEOBNRv6EHM in the pyseobnr package
    ...
    def _generate_waveform_class(self, **parameters):
        ...
        # Detect generic orbit parametrization
        generic_orbit = all(
            k in parameters_extra for k in ("separation", "energy", "momentum")
        )
        ...
        if not generic_orbit:
            ...
            # Generic orbits: remove bound-orbit params that are not needed
            self.waveform_dict.pop("meanPerAno", None)
            self.waveform_dict.pop("eccentricity", None)
